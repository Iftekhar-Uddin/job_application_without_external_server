...
export default function PaymentSuccessPage() {
  ...
  useEffect(() => {
    ...
    const fetchPayment = async () => {
      try {
        ...
        if (sessionId) {
          ...
          if (!res.ok) throw new Error("Stripe payment not found");
          paymentData = await res.json();
        } else if (jobId) {
          const res = await fetch(`/api/payment/sslcommerz/${jobId}`);
          if (!res.ok) throw new Error("SSLCOMMERZ payment not found");
          paymentData = await res.json();
        } else {
    ...
    if (paymentStatus === "success" && (sessionId || jobId)) {
      fetchPayment();
    } else {
      setLoading(false);
    }
  }, [sessionId, paymentStatus, jobId]);
  ...
  if (error || !payment) {
    return (
      <div className="max-w-7xl min-h-320px flex flex-col items-center justify-center text-slate-600 p-4 bg-white/70 rounded-md sm:rounded-lg">
        <div className="text-center max-w-md">
          <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <span className="text-2xl">⚠️</span>
          </div>
          <h2 className="text-xl font-semibold text-slate-800 mb-2">
            {error === "Payment not found or expired." ? "Payment Not Found" : "Payment Error"}
          </h2>
          ...
          <button
            onClick={() => router.push("/")}
            className="flex items-center gap-2 px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-sm transition mx-auto"
          >
          ...
          <section className="flex flex-col items-center justify-center text-center gap-5">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500">
                ...
                <span className="font-medium text-emerald-600">{displayProvider}</span>.
              </p>
            </div>
